...
if 'ex_correct' not in st.session_state: st.session_state.correct = 0
...
def get_cheer_message():
    messages = ["宜真，今天的妳也比昨天更進步了！加油！💙", "Process Engineer 的韓文實力正在穩定提升中！", "24/7 With Us! 練習累了就聽聽 TWS 的歌吧 🎶", "每一題的練習都是為了 10 月的 TOPIK 考照鋪路！", "今天也要保持應援藍的好心情喔！💎"]
...
if st.session_state.show_report:
    acc = (st.session_state.correct / st.session_state.ex_total * 100) if st.session_state.ex_total > 0 else 0
    st.markdown(f"""<div class="report-box"><h3 style='text-align: center; color: #007FFF;'>📊 Excel 複習結算</h3><p style='text-align: center; font-size: 20px;'>準確率：{acc:.1f}% ({st.session_state.correct}/{st.session_state.ex_total})</p></div>""", unsafe_allow_html=True)
    if st.session_state.wrong_items:
        ...
        if st.button("🔥 針對錯題重新複習"):
            ...
            st.session_state.wrong_items = []; st.session_state.ex_total = 0; st.session_state.correct = 0; st.session_state.show_report = False; st.rerun()
    if st.button("🔄 開啟全新複習"):
        st.session_state.ex_total = 0; st.session_state.correct = 0; st.session_state.wrong_items = []; st.session_state.pools = {"單字": [], "文法": [], "發音": []}; st.session_state.show_report = False; st.rerun()
    st.stop()
...
with c_dq1:
    if st.button("驗證翻譯"):
        if u_in_dq:
            if clean_text(u_in_dq) == clean_text(dq['kr']): st.success(f"⭕ 正確：{dq['kr']}"); st.balloons()
            else: st.error(f"❌ 錯誤！答案：{dq['kr']}"); play_audio(dq['kr'])
with c_dq2:
    if st.button("換下一題翻譯"):
...
if not df.empty:
    ...
    sel_ch = st.multiselect("篩選章節：", all_ch, default=all_ch) # 預設全選
        ...
        with tab:
            ...
            if p:
                ...
                if "閃卡" in study_mode:
                    ...
                    u_in = st.text_input("輸入答案回答", key=f"in_{target_cat}_{len(p)}")
                    if st.button("驗證", key=f"b_{target_cat}"):
                        is_ok = clean_text(u_in) == clean_text(str(item['kr']))
                        st.session_state.ex_total += 1
                        if is_ok: st.success("⭕ 正確！"); st.session_state.correct += 1
